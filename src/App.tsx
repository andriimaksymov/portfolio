  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/[0.08] bg-ink/70 backdrop-blur-xl">
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:px-6 lg:px-8" aria-label="Primary navigation">
        <a href="#top" className="flex items-center gap-3 rounded-lg outline-none focus-visible:ring-2 focus-visible:ring-cyan-300">
          <span className="flex h-9 w-9 items-center justify-center rounded-lg border border-cyan-300/30 bg-cyan-300/10 text-cyan-100">
        ...
        <div className="hidden items-center gap-1 md:flex">
          {navItems.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="rounded-lg px-3 py-2 text-sm font-medium text-slate-300 transition hover:bg-white/[0.06] hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300"
            >
              {item.label}
            </a>
          ))}
...
function Hero() {
  return (
    <section id="top" className="relative overflow-hidden px-5 pt-28 sm:px-6 lg:px-8">
      ...
      <div className="absolute inset-0 -z-10 animate-aurora bg-aurora opacity-80" aria-hidden="true" />
      <div className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-7xl items-center gap-12 pb-16 pt-12 lg:grid-cols-[1fr_0.9fr] lg:pb-24">
        <motion.div initial="hidden" animate="visible" variants={stagger}>
          <motion.div
            className="mb-6 inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/[0.055] px-3 py-2 text-sm font-medium text-slate-200 backdrop-blur"
            variants={fadeUp}
          >
            <MapPin className="h-4 w-4 text-emerald-200" aria-hidden="true" />
            {profile.location}
          </motion.div>
          <motion.p className="text-sm font-semibold text-cyan-200" variants={fadeUp}>
            {profile.name}
          </motion.p>
          <motion.h1 className="mt-4 max-w-4xl text-4xl font-semibold leading-[1.05] text-white md:text-6xl lg:text-7xl" variants={fadeUp}>
            {profile.headline}
          </motion.h1>
          <motion.p className="mt-6 max-w-2xl text-lg leading-8 text-slate-300 md:text-xl" variants={fadeUp}>
...
function About() {
  const items = [
    "Builds greenfield platforms from scratch with scalable frontend architecture.",
    "Modernizes legacy systems into performant, type-safe React architectures.",
    "Works closely with product managers and UX/UI designers.",
    "Owns frontend delivery from technical decisions to CI/CD and deployment.",
  ...
  return (
    <SectionWrapper
      ...
      description="Andrii is a Senior Front-End React Developer with experience across architecture, system design, frontend modernization, performance optimization, SEO, and scalable UI systems."
    >
      <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
        <AnimatedCard className="p-7 md:p-8">
          <Sparkles className="mb-6 h-8 w-8 text-cyan-200" aria-hidden="true" />
          <p className="text-2xl font-semibold leading-snug text-white">
            Product-minded engineering for teams that need modern frontend systems to move faster without losing quality.
          </p>
          <p className="mt-5 leading-8 text-slate-300">
            The work spans technical direction, architecture decisions, performance, SEO, code review, mentorship, and delivery workflows.
          </p>
        </AnimatedCard>
        <motion.div
        ...
        >
          {items.map((item) => (
            <motion.div
              key={item}
              className="flex gap-4 rounded-lg border border-white/10 bg-white/[0.04] p-5 backdrop-blur"
              variants={fadeUp}
            >
          ...
          <motion.div key={group.title} variants={fadeUp}>
            <AnimatedCard className="h-full">
              <div className="mb-5 flex items-center justify-between gap-4">
                ...
                <span className="flex h-10 w-10 items-center justify-center rounded-lg border border-white/10 bg-white/[0.04] font-mono text-sm text-cyan-200">
                  {icons[index]}
                </span>
              </div>
      ...
      >
        {certificates.map((certificate) => (
          <motion.div key={certificate} variants={fadeUp}>
            <AnimatedCard className="h-full p-5">
              <CheckCircle2 className="mb-4 h-5 w-5 text-emerald-200" aria-hidden="true" />
              <p className="text-sm font-semibold leading-6 text-white">{certificate}</p>
            </AnimatedCard>
          </motion.div>
        ))}
      </motion.div>
...
function Contact() {
  return (
    <SectionWrapper id="contact" className="pb-10">
      <div className="overflow-hidden rounded-lg border border-white/10 bg-gradient-to-br from-white/[0.085] via-white/[0.045] to-emerald-300/[0.06] p-6 shadow-lift backdrop-blur-xl md:p-10 lg:p-12">
        <div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end">
          <div>
            <p className="mb-4 text-sm font-semibold text-cyan-200">Contact</p>
            <h2 className="max-w-3xl text-3xl font-semibold text-white md:text-5xl">Let&apos;s build something scalable, fast, and useful.</h2>
            <p className="mt-5 max-w-2xl text-base leading-8 text-slate-300 md:text-lg">
              Companies and teams can contact Andrii for senior frontend, React, Next.js, architecture, or product-focused engineering roles.
            </p>
          </div>
      ...
      <footer className="mx-auto flex max-w-7xl flex-col gap-3 py-8 text-sm text-slate-500 md:flex-row md:items-center md:justify-between">
        <p>{profile.name}</p>
        <a className="rounded-lg text-slate-400 transition hover:text-cyan-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300" href={`mailto:${profile.email}`}>
          {profile.email}
        </a>
      </footer>
    </SectionWrapper>
